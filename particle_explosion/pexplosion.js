...
function loadthreeexp(){
  ...
  function createbox(X,Y,Z,j){
    ...
    group.add( particleSystem );
      TweenMax.to(particleSystem.scale, 5, { y:particleSystem.scale.y + (Math.random() * ( j%10==0  ?  -1 : 1  ))*300,
              x:particleSystem.scale.x + (Math.random() * ( j%10==0  ?  -1 : 1  ))*600,
            ease: Cubic.easeInOut, yoyo:true,repeat:-1
           });
      TweenMax.to(particleSystem.rotation, 1, { y:particleSystem.rotation.y + (Math.random() * ( j%10==0  ?  -1 : 1  ))*10,
             x:particleSystem.rotation.x + (Math.random() * ( j%10==0  ?  -1 : 1  ))*100,
           ease: Cubic.easeInOut, yoyo:true,repeat:-1
      });
  }
  function init() {
